...
const HowItWorksNew: React.FC = () => {
    ...
    return (
        <section ref={ref} className="relative py-24 sm:py-32 lg:py-40 bg-gradient-to-b from-slate-50 to-white overflow-hidden">
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl relative z-10">
                {/* Header */}
                <div className={`text-center mb-20 lg:mb-28 transition-all duration-1000 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200 mb-6 shadow-sm">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                        <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Einfacher Prozess</span>
                    </div>
                    <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-slate-900 mb-6 leading-[1.1]">
                    ...
                    <p className="text-xl lg:text-2xl text-slate-600 max-w-3xl mx-auto font-medium">
                                ...
                                <div className={`flex flex-col lg:flex-row items-center gap-8 lg:gap-16 ${
                                    ...
                                    <div className={`flex-1 w-full lg:w-1/2 ${isLeft ? 'lg:pr-8' : 'lg:pl-8'}`}>
                                        <div className="relative">
                                            ...
                                            <div 
                                                className={`absolute -inset-8 rounded-full opacity-20 transition-all duration-500 ${
                                            ...
                                            <div className={`relative rounded-3xl p-12 lg:p-16 bg-white border-2 transition-all duration-500 ${
                                                isHovered 
                                                ...
                                                {/* Step Number - Large */}
                                                <div className={`absolute -top-6 ${isLeft ? '-left-6' : '-right-6'} w-24 h-24 rounded-2xl bg-gradient-to-br ${getColorClasses(step.primaryColor, step.secondaryColor, 'gradient')} flex items-center justify-center transform transition-all duration-500 ${
                                                ...
                                                <div className="flex justify-center mb-8">
                                                    <div className={`w-32 h-32 rounded-3xl bg-gradient-to-br ${getColorClasses(step.primaryColor, step.secondaryColor, 'gradient')} flex items-center justify-center text-6xl transform transition-all duration-500 ${
                                                ...
                                                <div className="flex justify-center">
                                                    <div className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r ${getColorClasses(step.primaryColor, step.secondaryColor, 'gradient')} text-white shadow-lg`}>
                                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                        </svg>
                                                        <span className="text-sm font-black">{step.time}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className={`flex-1 w-full lg:w-1/2 ${isLeft ? 'lg:pl-8' : 'lg:pr-8'}`}>
                                        <div className="space-y-6">
                                            {/* Title */}
                                            <h3 className={`text-4xl lg:text-5xl font-black mb-4 transition-colors duration-300 ${
                                            ...
                                            <div className="space-y-4">
                                                {step.benefits.map((benefit, benefitIndex) => (
                                                    ...
                                                    >
                                                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${getColorClasses(step.primaryColor, step.secondaryColor, 'gradient')} flex items-center justify-center flex-shrink-0 transform transition-all duration-300 ${
                                                        ...
                                                        <span className="text-base lg:text-lg font-semibold text-slate-800">{benefit}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                ...
                {/* CTA Section */}
                <div className={`text-center mt-24 lg:mt-32 transition-all duration-1000 delay-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    <button 
                        onClick={() => openQuoteModal()}
                        className="group relative inline-flex items-center gap-4 bg-gradient-to-r from-slate-900 to-slate-800 hover:from-slate-800 hover:to-slate-700 text-white font-black text-lg lg:text-xl px-12 lg:px-16 py-6 lg:py-7 rounded-2xl shadow-2xl hover:shadow-[0_30px_60px_rgba(0,0,0,0.3)] transition-all duration-300 hover:-translate-y-1 overflow-hidden"
                    >
                    ...
                    {/* Trust Indicators */}
                    <div className="flex flex-wrap items-center justify-center gap-8 lg:gap-12 mt-8 text-sm font-bold text-slate-600">
