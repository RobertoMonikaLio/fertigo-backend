...
const CustomerHeader: React.FC = () => {
    ...
    // Read real customer data from localStorage
    const customerData = (() => {
        try {
    ...
    return (
        <div className="fixed top-0 left-0 w-full z-40 flex flex-col shadow-sm font-sans">
                    ...
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        <div className="relative" ref={userMenuRef}>
                            <button onClick={() => setIsUserMenuOpen(prev => !prev)} className="flex items-center gap-2 p-1 rounded-full hover:bg-slate-100 transition-colors">
                                <div className="w-9 h-9 bg-gradient-to-br from-primary-400 to-primary-600 rounded-full flex items-center justify-center font-bold text-white border-2 border-white shadow-sm text-sm">
                                    {initials}
                                </div>
                            ...
                            {isUserMenuOpen && (
                                <div className="absolute top-full right-0 mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-2xl z-10 animate-fade-in">
                                    <div className="p-4 border-b border-slate-200">
                                        ...
                                        <p className="text-xs text-slate-500 truncate font-medium">{displayEmail}</p>
                                    </div>
                                    <ul className="py-2">
                                        <li>
                                            <Link to="/kunden/einstellungen" onClick={() => setIsUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-slate-700 hover:bg-slate-50 font-medium transition-colors">
                                                <UserIcon className="w-5 h-5 text-slate-400" />
                                                <span>Mein Profil</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2.5 text-red-600 hover:bg-red-50 font-medium transition-colors">
            ...
            {/* Mobile Overlay */}
            <div
                className={`fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm lg:hidden transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
            ...
            <div className={`fixed top-0 left-0 bottom-0 w-80 bg-white shadow-2xl z-50 transform transition-transform duration-300 lg:hidden flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-5 h-20 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-primary-400 to-primary-600 rounded-full flex items-center justify-center font-bold text-white shadow-md">
                            {initials}
                        </div>
                        <div>
                    ...
                    <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 rounded-full hover:bg-white hover:shadow-sm transition-all text-slate-500">
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50 space-y-3">
                    ...
                    <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-slate-600 font-bold hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors">
