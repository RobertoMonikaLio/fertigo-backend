...
const mockPartners: Partner[] = [
    { id: 1, companyName: 'Maler Müller AG', contactPerson: { anrede: 'Herr', name: 'Markus Müller', position: 'Geschäftsführer' }, email: 'partner@offerten.ch', phone: '044 123 45 67', mobile: '079 123 45 67', services: ['Malerarbeiten', 'Gipserarbeiten'], location: 'Musterstrasse 1, 8004 Zürich', kanton: 'ZH', status: 'Aktiv', registrationDate: '15. Januar 2023', uid: 'CHE-111.222.333', website: 'www.maler-mueller.ch', employees: '15-20', insuranceProvider: 'AXA Winterthur', insuranceDocuments: [{ name: 'Police_Haftpflicht_2024.pdf', url: '#' }] },
    { id: 2, companyName: 'Gartenbau Frischgrün', contactPerson: { anrede: 'Frau', name: 'Sabine Grün', position: 'Inhaberin' }, email: 's.gruen@frischgrün.ch', phone: '041 234 56 78', mobile: '078 234 56 78', services: ['Gartenpflege', 'Gartenbau'], location: 'Gartenweg 2, 6000 Luzern', kanton: 'LU', status: 'Aktiv', registrationDate: '02. März 2023', uid: 'CHE-444.555.666', website: 'www.garten-gruen.ch', employees: '5-10', insuranceProvider: 'Helvetia', insuranceDocuments: [{ name: 'Nachweis_Betriebshaftpflicht.pdf', url: '#' }] },
    { id: 3, companyName: 'Elektro Blitz AG', contactPerson: { anrede: 'Herr', name: 'Peter Schnell', position: 'Projektleiter' }, email: 'p.schnell@elektroblitz.ch', phone: '044 345 67 89', mobile: '077 345 67 89', services: ['Elektroinstallationen'], location: 'Industriestrasse 10, 8050 Oerlikon', kanton: 'ZH', status: 'In Prüfung', registrationDate: '20. Juli 2024', uid: 'CHE-777.888.999', website: 'www.elektroblitz.ch', employees: '25+', insuranceProvider: 'Zürich Versicherung', insuranceDocuments: [{ name: 'Police_2024.pdf', url: '#' }, { name: 'Zusatzversicherung.pdf', url: '#' }] },
    { id: 4, companyName: 'Reinigung Perfekt GmbH', contactPerson: { anrede: 'Frau', name: 'Maria Saubermann', position: 'Teamleiterin' }, email: 'maria@perfekt.ch', phone: '031 456 78 90', mobile: '076 456 78 90', services: ['Reinigung', 'Umzugsreinigung'], location: 'Hauptstrasse 5, 3000 Bern', kanton: 'BE', status: 'Aktiv', registrationDate: '10. Mai 2022', uid: 'CHE-123.456.789', website: 'www.perfekt-reinigung.ch', employees: '30+', insuranceProvider: 'Baloise', insuranceDocuments: [{ name: 'Police_Betriebshaftpflicht_aktuell.pdf', url: '#' }] },
    { id: 5, companyName: 'Sanitär Express', contactPerson: { anrede: 'Herr', name: 'Hans Wasser', position: 'Inhaber' }, email: 'h.wasser@express.ch', phone: '061 567 89 01', mobile: '079 567 89 01', services: ['Sanitär'], location: 'Wasserweg 8, 4051 Basel', kanton: 'BS', status: 'Gesperrt', registrationDate: '01. Februar 2022', uid: 'CHE-987.654.321', website: 'www.sanitaer-express.ch', employees: '2-5', insuranceProvider: 'Generali', insuranceDocuments: [] },
    { id: 6, companyName: 'Bodenleger Profi GmbH', contactPerson: { anrede: 'Herr', name: 'Klaus Boden', position: 'Geschäftsführer' }, email: 'k.boden@profi.ch', phone: '052 678 90 12', mobile: '078 678 90 12', services: ['Bodenleger'], location: 'Parkettstrasse 3, 8400 Winterthur', kanton: 'ZH', status: 'Aktiv', registrationDate: '18. August 2023', uid: 'CHE-246.813.579', website: 'www.boden-profi.ch', employees: '5-10', insuranceProvider: 'AXA Winterthur', insuranceDocuments: [{ name: 'versicherung_2024.pdf', url: '#' }] },
];
                ...
                <div className="p-6 overflow-y-auto space-y-6">
                    <DetailSection title="Firmendetails" icon={<BriefcaseIcon className="w-5 h-5 text-slate-500" />}>
                        <div className="space-y-1 text-sm">
                            ...
                            <DetailItem label="Webseite" value={<a href={`https://${partner.website}`} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline">{partner.website}</a>} />
                    ...
                    <DetailSection title="Kontaktperson" icon={<UserIcon className="w-5 h-5 text-slate-500" />}>
                        <div className="space-y-1 text-sm">
                            ...
                            <DetailItem label="E-Mail" value={<a href={`mailto:${partner.email}`} className="text-primary-600 hover:underline">{partner.email}</a>} />
                            <DetailItem label="Telefon" value={<a href={`tel:${partner.phone}`} className="text-primary-600 hover:underline">{partner.phone}</a>} />
                            <DetailItem label="Mobile" value={<a href={`tel:${partner.mobile}`} className="text-primary-600 hover:underline">{partner.mobile}</a>} />
                        </div>
                    </DetailSection>
                        ...
                        <div className="space-y-1 text-sm">
                            ...
                            <div className="py-1.5 grid grid-cols-3 gap-2">
                                ...
                                <div className="text-slate-800 font-medium col-span-2">
                                    {partner.insuranceDocuments.length > 0 ? (
...
const AdminPartnersPage: React.FC = () => {
    ...
    const availableServices = useMemo(() => {
        ...
        mockPartners.forEach(p => p.services.forEach(s => allServices.add(s)));
    ...
    const filteredPartners = useMemo(() => {
        return partners.filter(p => {
            ...
            const matchesService = serviceFilter === 'Alle' || p.services.includes(serviceFilter);
    ...
    const exportPartnersToCSV = () => {
        const header = "ID,Firmenname,Ansprechperson,E-Mail,Telefon,Mobile,Standort,Kanton,Status,Registriert,UID,Webseite,Mitarbeiter,Dienstleistungen\n";
                                ...
                                <table className="w-full text-sm text-left text-slate-600">
                                    ...
                                    <tbody>
                                        {filteredPartners.map(partner => (
                                            <tr key={partner.id} className="bg-white border-b hover:bg-slate-50 cursor-pointer" onClick={() => handleViewDetails(partner)}>
                                                ...
                                                <td className="px-6 py-4">{partner.location.split(', ')[1]}</td>
                                 ...
                                 {filteredPartners.length === 0 && (
                                    <div className="text-center p-12">
                                        ...
                                        <h3 className="mt-4 text-lg font-semibold text-slate-700">Keine Partner gefunden</h3>
                                        <p className="mt-1 text-sm text-slate-500">Ihre Suche ergab keine Treffer. Versuchen Sie, die Filter anzupassen.</p>
                                    </div>
                                )}
                            </div>
                        </div>
